 ...
 */
const oid = () => new mongoose_1.default.Types.ObjectId();
const signup = (over = {}) => (Object.assign({ signupId: oid(), userId: oid(), name: "Jugador", isGuest: false }, over));
const team = (players, over = {}) => (Object.assign({ teamId: oid(), name: "Equipo", players }, over));
let passed = 0;
...
check("pool-based: payload válido arma los equipos", () => {
    ...
    const result = (0, roster_1.validateRosterPayload)({
        payload,
        teamFormationMode: constants_1.TEAM_FORMATION_MODES.CREATOR_FORMED,
        teamSize: 2,
        existingTeams: [],
        individualSignups: [s1, s2, s3]
    });
    (0, assert_1.default)(result.ok, "debería validar OK");
    if (result.ok) {
        assert_1.default.strictEqual(result.teams.length, 1);
        assert_1.default.strictEqual(result.teams[0].players.length, 1);
        assert_1.default.strictEqual(result.teams[0].players[0].name, "Ana");
    }
});
...
check("pool-based: jugador fuera del pool devuelve 409", () => {
    ...
    const result = (0, roster_1.validateRosterPayload)({
        payload,
        teamFormationMode: constants_1.TEAM_FORMATION_MODES.RANDOM,
        teamSize: 2,
        existingTeams: [],
        individualSignups: [s1]
    });
    (0, assert_1.default)(!result.ok, "debería fallar");
    if (!result.ok) {
        assert_1.default.strictEqual(result.status, 409);
    }
});
...
check("user-formed: falta un jugador devuelve 409", () => {
    ...
    const result = (0, roster_1.validateRosterPayload)({
        payload,
        teamFormationMode: constants_1.TEAM_FORMATION_MODES.USER_FORMED,
        teamSize: 2,
        existingTeams: [t1],
        individualSignups: []
    });
...
check("user-formed: equipo incompleto devuelve 400", () => {
    ...
    (0, assert_1.default)(!result.ok, "debería fallar");
    if (!result.ok)
        assert_1.default.strictEqual(result.status, 400);
});
...
check("dos invitados homónimos se distinguen por signupId", () => {
    ...
    (0, assert_1.default)(result.ok, "debería validar OK: son dos invitados distintos");
    if (result.ok) {
        assert_1.default.strictEqual(result.teams.length, 2);
    }
});
...
check("jugadores legacy sin signupId matchean por playerId", () => {
    ...
    (0, assert_1.default)(result.ok, "debería validar OK vía el fallback de playerKey");
    if (result.ok) {
        const withPlayer = result.teams.find((t) => t.players.length === 1);
        assert_1.default.strictEqual(withPlayer.players[0].name, "Legacy");
    }
});
...
check("playerKey prioriza signupId sobre playerId y nombre", () => {
    const sid = oid();
    const pid = oid();
    assert_1.default.strictEqual((0, roster_1.playerKey)({ signupId: sid, playerId: pid, name: "X" }), `s:${sid}`);
    assert_1.default.strictEqual((0, roster_1.playerKey)({ playerId: pid, name: "X" }), `u:${pid}`);
    assert_1.default.strictEqual((0, roster_1.playerKey)({ name: "  Ana  " }), "g:ana");
});
console.log(`\n${passed} check(s) OK`);
